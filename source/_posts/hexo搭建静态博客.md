...
创建一个和你用户名相同且小写的仓库，后面加.github.io，只有这样，将来要部署到GitHub page的时候，才会被识别，也就是xxxx.github.io.
...
![img](https://www.baiyp.ren/images/hexo/domain01.jpg)

其中，192.30.252.153 和 192.30.252.154 是GitHub的服务器地址。
**注意，解析线路选择默认**，不要像我一样选境外。这个境外是后面来做国内外分流用的,在后面的博客中会讲到。记得现在选择**默认**！！ 

![img](https://www.baiyp.ren/images/hexo/domain02.jpg)

登录GitHub，进入之前创建的仓库，点击settings，设置Custom domain，输入你的域名`baiyp.ren`

![img](https://www.baiyp.ren/images/hexo/domain03.jpg)

然后在你的博客文件source中创建一个名为CNAME文件，不要后缀。写上你的域名。 

![img](https://www.baiyp.ren/images/hexo/domain04.jpg)
...
![img](https://www.baiyp.ren/images/hexo/hexo03.jpg)
...
![img](https://www.baiyp.ren/images/hexo/hexo04.jpg)
...
例如,需要打开tags和categories菜单,修改主题的配置文件_config.yml,找到menu,去掉前面井号:

![image-20221121223333836](/Users/boguotong/myblog/source/_posts/hexo搭建静态博客.assets/image-20221121223333836.png)

执行:会在source目录下生成对应文件夹.
...
最后一步，打开source目录下各页面对应的index.md文件，编辑如下内容，title和date是默认生成的，增加type后重新部署即可.

![image-20221121223609014](/Users/boguotong/myblog/source/_posts/hexo搭建静态博客.assets/image-20221121223609014.png)



如果需要添加自定义菜单,在主题配置文件的menu菜单栏添加一个 `Yourdiy : /yourdiy`，注意冒号后面要有空格，以及前面的空格要和menu中默认的保持整齐。然后在`languages`文件夹中，找到`zh-CN.yml`，在index中添加`yourdiy: '中文意思'`就可以显示中文了。 
...
我关注的比如，阮一峰的网络日志，月光博客，知乎精选等，都很不错。当然，还有我！！赶快关注我吧！你值得拥有：http://baiyp.ren/atom.xml
...
里我就改了一个`links`，可以添加友链。注意空格要对！不然会报错！ 
...
![img](https://www.baiyp.ren/images/hexo/source01.jpg)

也就是上传的是在本地目录里自动生成的`.deploy_git`里面。

其他文件 ，包括我们写在source 里面的，和配置文件，主题文件，都没有上传到github

![img](https://www.baiyp.ren/images/hexo/source02.jpg)
...
![img](https://www.baiyp.ren/images/hexo/source03.jpg)

然后在这个仓库的settings中，选择默认分支为hexo分支（这样每次同步的时候就不用指定分支，比较方便）。

![img](https://www.baiyp.ren/images/hexo/source04.jpg)
...
![img](https://www.baiyp.ren/images/hexo/source05.jpg)
...
### 5. coding page上部署实现国内外分流 (已废弃)

**coding.net已经不支持pages了!!!**

**近期，因 CODING 产品战略调整，决定将于 2021 年 12 月 30 日起不再提供网站托管服务的使用入口，请您直接前往腾讯云 Serverless 控制台使用，并对站点进行管理与维护。此过程中，您无需迁移存量站点，站点的访问不会受到影响，如需访问项目代码，可前往 CODING 代码仓库或您的第三方代码仓库中查看。**
...
deploy:
  type: git
  repo: 
    coding: git@e.coding.net:yourgithubemail/yourgithubemail.git,master
...
![img](https://www.baiyp.ren/images/hexo/coding01.jpg)

#### **阿里云添加解析**

![img](https://www.baiyp.ren/images/hexo/coding02.jpg)
...
![img](https://www.baiyp.ren/images/hexo/coding03.jpg)
...
![img](https://www.baiyp.ren/images/hexo/coding04.jpg)
...
### 6. Gitee上部署实现国内外分流 (代替coding)

#### **申请gitee账户，新建项目**

先申请一个账户，然后创建新的项目，这一步项目名称应该是随意的,但是仓库的路径要和用户名（个人空间地址）一致,这样访问地址就不会生成[二级域名](https://so.csdn.net/so/search?q=二级域名&spm=1001.2101.3001.7020)了,否则点击tags等会404

![image-20221122220304440](/Users/boguotong/myblog/source/_posts/hexo搭建静态博客.assets/image-20221122220304440.png)

#### **添加ssh key**

这一步跟github一样。注意不是具体仓库中的公钥,而是个人设置中的公钥

![image-20221122213640735](/Users/boguotong/myblog/source/_posts/hexo搭建静态博客.assets/image-20221122213640735.png)

![image-20221122213338444](/Users/boguotong/myblog/source/_posts/hexo搭建静态博客.assets/image-20221122213338444.png)
...
deploy:
- type: git
  # 同时推送到github和gitee两个代码仓库
  repo: git@github.com:blydd/blydd.github.io.git
  branch: master
- type: git	  
  repo: git@gitee.com:blydd/blydd.gitee.io.git
  branch: master
```
...
![img](https://www.baiyp.ren/images/hexo/seo01.jpg)
...
![img](https://www.baiyp.ren/images/hexo/seo02.jpg)
...
![img](https://www.baiyp.ren/images/hexo/pinglun01.jpg)
...
![img](https://www.baiyp.ren/images/hexo/pinglun02.jpg)
...
![img](https://www.baiyp.ren/images/hexo/bdtj01.jpg)
...
![img](https://www.baiyp.ren/images/hexo/leanCloud01.jpg)

在`存储`中创建Class，命名为Counter, 

![img](https://www.baiyp.ren/images/hexo/leanCloud02.jpg)
...
>
> 本文永久链接是：[http://www.baiyp.ren/HEXO%E5%8D%9A%E5%AE%A2%E6%90%AD%E5%BB%BA.html](
